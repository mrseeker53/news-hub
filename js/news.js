...
const displayNewsDatails = async (newsId) => {
    ...
    totalNews.innerHTML = `
    <h5>${newsId.length ? newsId.length : 'No'} News Found</h5>
    `;
            ...
            <div class="col-md-8 col-xs-1">
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center px-3 pt-3">
                        <img style="height: 60px; width: 60px;" class="img-fluid rounded me-3" src="${news.author.img}"
                        <h5 class="pe-5 me-5">${news.author.name}</h5>
                        <span class="px-5 mx-5">${news.total_view}</span>
                        <button onclick="loadNewsInfo('${news._id}')" class="btn btn-primary px-5 d-flex ms-auto" data-bs-toggle="modal" data-bs-target="#newsInfoModal">Info</button>
                    </div>
                </div>
            </div>
        </div>
